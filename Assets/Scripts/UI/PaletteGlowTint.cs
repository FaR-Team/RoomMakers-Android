...
public class PaletteGlowTint : MonoBehaviour
{
    ...
    [Tooltip("Automatically applies UI/GlowTint shader so the glow takes the exact palette color instead of multiplying black pixels.")]
    public bool useGlowShader = true;

    [Tooltip("Enable if your glow sprite has dark/black borders on a white background.")]
    public bool invertDarkToAlpha = false;
    ...
    private void EnsureGlowMaterial(Image img)
    {
        ...
        glowMaterial.SetFloat("_InvertAlpha", invertDarkToAlpha ? 1f : 0f);
